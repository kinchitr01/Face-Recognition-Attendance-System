...
class Admin_teacher:
    def __init__(self,root):
        ...
        self.root.iconbitmap(r"D:\Face Recognition Attendence System\Images\icon_atendance.ico")
       ...
       # bg_img=Label(self.root,image=self.photoimg2)
        ...
        #SRKI LOGO
        img=Image.open(r"D:\Face Recognition Attendence System\Images\SRKI_logo.png")
        ...
        title_lbl=Label(text="Face Recongnition Attendance System",font=("times new roman",35,"bold"), bg="sky blue",fg="red")
        title_lbl.place(x=370,y=10,width=800,height=150)
        title_lbl1=Label(text="By Kinchit Raval,Manthan Makwana,Krishna Kapadia ",font=("times new roman",20,"bold"), bg="sky blue",fg="red")
        title_lbl1.place(x=410,y=120,width=700,height=50)

        #Sarvajanik LOGO
        img1=Image.open(r"D:\Face Recognition Attendence System\Images\Sarvajanik_logo.png")
        ...
        #left label frame
        left_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Teacher Registration",font=("times new roman",12,"bold"))
        ...
        sdetail_frame=LabelFrame(left_frame,bd=2,bg="white",relief=RIDGE,text="Teacher Details",font=("times new roman",12,"bold"))
        sdetail_frame.place(x=5,y=70,width=700,height=480)
        #sdetail_frame.place(x=5,y=160,width=700,height=390)

         #TeachertName
        studID_label=Label(sdetail_frame,text="Teacher Name:",font=("times new roman",12,"bold"),bg="white")
        studID_label.grid(row=0,column=0,padx=2,pady=20)

        studname_entry=Entry(sdetail_frame,textvariable=self.tname,width=20,font=("times new roman",12,"bold"))
        ...
        gender_label.grid(row=0,column=2,padx=2,pady=20)
        ...
        gen_combo.current(0)
        gen_combo.grid(row=0,column=3,padx=20)
        ...
        take_photo_btn=Button(btn_frame,text="TAKE PHOTO SAMPLE",width=25,font=("times new roman",12,"bold"),bg="blue",fg="white")
        take_photo_btn.grid(row=0,column=0)

        update_photo_btn=Button(btn_frame,text="UPDATE PHOTO SAMPLE",width=25,font=("times new roman",12,"bold"),bg="blue",fg="white")
        update_photo_btn.grid(row=0,column=1)

        add_admin_btn=Button(btn_frame,text="ADD ADMIN",width=25,font=("times new roman",12,"bold"),bg="blue",fg="white")
        add_admin_btn.grid(row=0,column=2)

        add_teacher_btn=Button(btn_frame,text="BACK",width=25,font=("times new roman",12,"bold"),bg="blue",fg="white")
        add_teacher_btn.grid(row=1,column=1)        
        ...
        #Search System
        search_frame=LabelFrame(right_frame,bd=2,bg="white",relief=RIDGE,text="Teacher Information",font=("times new roman",12,"bold"))
        ...
        search_combo["values"]=("Select value","Teacher_ID","Department")
        search_combo.current(0)
        search_combo.grid(row=0,column=1,padx=2,pady=10)

        search_entry=Entry(search_frame,width=18,font=("times new roman",12,"bold"))
        ...
        show_btn=Button(search_frame,text="Show All",width=15,font=("times new roman",12,"bold"),bg="blue",fg="white")
        ...
        self.student_table=ttk.Treeview(table_frame,columns=("tid","tname","gender","dob","mobile","address","city","photo","dep"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.student_table.xview)
        scroll_y.config(command=self.student_table.yview)

        self.student_table.heading("tid",text="Teacher ID")
        ...
        self.student_table.column("tid",width=100)
    ...
    #functions
    def add_data(self):
        if self.tname.get()=="" or self.gen.get()=="Select Gender" or self.dob.get()=="" or self.email.get()=="" or self.mobile.get()=="" or self.address.get()=="" or self.city.get()=="" or self.dep.get()=="Select Departement":
            ...
            try:
                ...
                my_cursor.execute("insert into teacher (teacher_name,gender,dob,email,mobile,address,city,photo,department) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                                        self.tname.get(),
                                                                                                        self.gen.get(),
                                                                                                        self.dob.get(),
                                                                                                        self.email.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success","Teacher Registered!",parent=self.root)
    ...
    def get_data(self,event=""):
        ...
        self.dep.set(data[9])
        self.tname.set(data[1])
        self.gen.set(data[2])
        self.dob.set(data[3])
        self.email.set(data[4])
        self.mobile.set(data[5])
        self.address.set(data[6])
        self.city.set(data[7])
        self.var_radio_1.set(data[8])   
        ...
        if self.tname.get()=="" or self.gen.get()=="Select Gender" or self.dob.get()=="" or self.email.get()=="" or self.mobile.get()=="" or self.address.get()=="" or self.city.get()=="" or self.dep.get()=="Select Departement":
            ...
            try:
                ...
                if update>0:
                   ...
                   conn=mysql.connector.connect(host="localhost",username="root",password="kinchit123",database="attendance_system")
                   my_cursor=conn.cursor()
                   my_cursor.execute("update teacher set teacher_name=%s,gender=%s,dob=%s,email=%s,mobile=%s,address=%s,city=%s,photo=%s,department=%s where teacher_ID=%s",(
                    ...
                    if not update:
                        return
                messagebox.showinfo("Success","Teacher Details Updated!",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
